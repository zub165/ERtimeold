...
def generate_icon_from_png(png_path, size, output_path):
    """Generate PNG icon from source PNG at specified size"""
    try:
...
def main():
    png_path = "assets/icons/icon_AppStore_1024x1024.png"
    output_dir = "assets/icons"
    
    # Create output directory
    os.makedirs(output_dir, exist_ok=True)
    
    print("Generating iOS app icons from PNG source...")
    ...
    for name, size in icon_sizes.items():
        ...
        if generate_icon_from_png(png_path, size, output_path):
            success_count += 1
