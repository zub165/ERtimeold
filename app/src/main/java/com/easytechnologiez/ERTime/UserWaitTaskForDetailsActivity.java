...
public class UserWaitTaskForDetailsActivity extends AsyncTask<Void,Void,FeedbackResponse>
{
    ...
    Handler mHandler;
    UserWaitTaskForDetailsActivity(Context context   , ViewFeedbackListener listener2, PlaceInfo placeInfo )
    {
        ...
        if (infos != null && infos.getmStatus() !=null)
        {
         //   Toast.makeText(context,infos.getmMessage(), Toast.LENGTH_SHORT).show();
            ...
            if (infos.getFeedbackInfos().size()>0) {
                ...
                info.setmRating(infos.getFeedbackInfos().get(0).getRating());
                info.setUserWaitAvaliable(false);
                listener.showAverage(info);
            }
